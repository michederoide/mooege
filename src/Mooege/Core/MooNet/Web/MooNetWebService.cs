...
namespace Mooege.Core.MooNet.Web
{
    ...
    public class MooNetWebService : IWebService
    {
        ...
        {
            return (from client in PlayerManager.OnlinePlayers where client.Account.CurrentGameAccount.CurrentToon != null select string.Format("{0}", client.Account.CurrentGameAccount.CurrentToon.HeroNameField.Value)).ToList();
        }
    }
}
